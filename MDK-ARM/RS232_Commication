...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1)
    {
        ...
        {
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); // LED'i söndür
					HAL_UART_Transmit(&huart1, (uint8_t*)"\r\n", 2, HAL_MAX_DELAY);
        }

        // Alinan veriyi yanit olarak gönder
        HAL_UART_Transmit(&huart1, (uint8_t*)responseMsg, sizeof(responseMsg) - 1, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart1, (uint8_t*)rxData, 1, HAL_MAX_DELAY); // Tek bir karakter gönder
        HAL_UART_Transmit(&huart1, (uint8_t*)"\r\n", 2, HAL_MAX_DELAY); // Satir sonu ekle

        // Yeni alim için UART'i baslat
